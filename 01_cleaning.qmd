...
```{r}
organs_original <- read_csv("global-organ-donation_2018.csv", 
...
organs_original |>
  group_by(REPORTYEAR) |>
  summarise(n = n(), non_missing = sum(!is.na(`TOTAL Actual DD`)))|>
  ggplot(aes(x = REPORTYEAR)) +
  geom_col(aes(y = n), fill = "red") +
  geom_col(aes(y = non_missing), fill = "blue") 
...
```{r}
organs_complete <- organs_original |> complete(COUNTRY, REPORTYEAR = full_seq(min(REPORTYEAR):max(REPORTYEAR), 1))
  
organs_complete$
```

**Answer to the HW question **
The problem was that after 2015 we have (had) mainly data about countries
that reported the metrics (and no records) for the countries that haven't 
reported the counts.
Note: I have done code here, as later on the code we call imputeFeature function again.
::::
